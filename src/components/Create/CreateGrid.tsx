...
const CreateGrid = ({ images: initialImages, currentUserId }: Props) => {
  ...
  const handleDeleteImage = useCallback(async (imageId: string) => {
    ...
    if (success) {
      ...
      toast.error("Deleting Image", {
        description: "An error occurred while deleting the image, please try again later.",
      });
    }
  }, []);
  ...
  const renderImageCard = useCallback(
    (img: Image) => (
      <ImageCard
        ...
        prompt={img.status === "pending" || img.status === "running" ? "Generating..." : img.prompt}
  ...
  const { isConnected } = useSocket("http://localhost:5000", "db_update", handleDbUpdate);
  ...
  const grouped = useMemo(() => {
    ...
    const isThisWeek = (dateStr: string) => {
      ...
      return d >= start && d < end && !isToday(dateStr) && !isYesterday(dateStr);
    };
  ...
  if (!images || images.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[79vh]">
        <img src="/empty-imagen.png" alt="Empty image" className="w-75 h-auto object-cover rounded-lg mb-2" />
        <p className="text-neutral-400">Create images using the prompt below or upload your own images.</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-2 sm:px-2 w-full max-w-full space-y-6 h-full ">
      ...
      {Object.entries(grouped).map(([section, imgs]) => {
        if (!imgs || (section === "Older" && Object.keys(imgs as object).length === 0)) {
          return null;
        }

        return section !== "Older" ? (
          <div key={section} className="w-full">
            <h2 className="text-base sm:text-lg font-semibold mb-2 text-gray-700 text-left">{section}</h2>
          ...
          <div key={section}>
            <h2 className="text-base sm:text-lg font-semibold mb-2 text-gray-700 text-left">Older</h2>
            {Object.entries(imgs as { [date: string]: Image[] }).map(([date, dateImgs]) => (
              <div key={date} className="mb-8">
                <div className="font-medium text-gray-500 mb-2 pl-1">{date}</div>
